...
ToplingRocks_lib := topling-zip-table-reader
ToplingRocks_src := $(wildcard src/table/*.cc)
ToplingRocks_src += ${BUILD_ROOT}/git-version-topling_zip_table_reader.cc

ToplingTests_src += $(wildcard src/table/tests/*.cc)

LIB_TERARK_D := -L${ZipCoreLibDir} -lterark-zbs-${COMPILER}-d -lterark-fsa-${COMPILER}-d -lterark-core-${COMPILER}-d
LIB_TERARK_R := -L${ZipCoreLibDir} -lterark-zbs-${COMPILER}-r -lterark-fsa-${COMPILER}-r -lterark-core-${COMPILER}-r
...
${TarBall}: ${ToplingRocks_d} ${static_ToplingRocks_d} \
            ...
            $(wildcard ${ZipCoreLibDir}/libterark-*)
	rm -rf ${TarBall}
	mkdir -p ${TarBall}/lib
	mkdir -p ${TarBall}/bin
...
ifeq (${PKG_WITH_DBG},1)
	...
	cp -a ${ZipCoreLibDir}/libterark-zbs-{${COMPILER}-,}d${DLL_SUFFIX} ${TarBall}/lib
	cp -a ${ZipCoreLibDir}/libterark-fsa-{${COMPILER}-,}d${DLL_SUFFIX} ${TarBall}/lib
	cp -a ${ZipCoreLibDir}/libterark-core-{${COMPILER}-,}d${DLL_SUFFIX} ${TarBall}/lib
  ifeq (${PKG_WITH_STATIC},1)
	mkdir -p ${TarBall}/lib_static
	cp -a ${BUILD_ROOT}/lib/lib${ToplingRocks_lib}-{${COMPILER}-,}d.a ${TarBall}/lib_static
	cp -a ${ZipCoreLibDir}/libterark-zbs-{${COMPILER}-,}d.a ${TarBall}/lib_static
	cp -a ${ZipCoreLibDir}/libterark-fsa-{${COMPILER}-,}d.a ${TarBall}/lib_static
	cp -a ${ZipCoreLibDir}/libterark-core-{${COMPILER}-,}d.a ${TarBall}/lib_static
  endif
endif
ifeq (${PKG_WITH_STATIC},1)
	mkdir -p ${TarBall}/lib_static
	cp -a ${BUILD_ROOT}/lib/lib${ToplingRocks_lib}-{${COMPILER}-,}r.a ${TarBall}/lib_static
	cp -a ${ZipCoreLibDir}/libterark-zbs-{${COMPILER}-,}r.a ${TarBall}/lib_static
	cp -a ${ZipCoreLibDir}/libterark-fsa-{${COMPILER}-,}r.a ${TarBall}/lib_static
	cp -a ${ZipCoreLibDir}/libterark-core-{${COMPILER}-,}r.a ${TarBall}/lib_static
endif
ifeq (${PKG_WITH_ROCKSDB},1)
  ...
  ifeq (${PKG_WITH_STATIC},1)
	...
	cp -a ${ZipCoreLibDir}/libterark-zbs-{${COMPILER}-,}r${DLL_SUFFIX} ${TarBall}/lib
	cp -a ${ZipCoreLibDir}/libterark-fsa-{${COMPILER}-,}r${DLL_SUFFIX} ${TarBall}/lib
	cp -a ${ZipCoreLibDir}/libterark-core-{${COMPILER}-,}r${DLL_SUFFIX} ${TarBall}/lib
	cp src/table/topling_zip_table.h           ${TarBall}/include/table
	cp src/table/topling_zip_weak_function.h   ${TarBall}/include/table
...
${ddir}/%.exe: ${ddir}/%.o
	@echo Linking ... $@
	${LD} ${LDFLAGS} -o $@ $< -L${BUILD_ROOT}/lib -L${ZipCoreLibDir}/lib -lterark-zbs-${COMPILER}-d -lterark-fsa-${COMPILER}-d -lterark-core-${COMPILER}-d ${LIBS}

${rdir}/%.exe: ${ddir}/%.o
	@echo Linking ... $@
	${LD} ${LDFLAGS} -o $@ $< -L${BUILD_ROOT}/lib -L${ZipCoreLibDir}/lib -lterark-zbs-${COMPILER}-r -lterark-fsa-${COMPILER}-r -lterark-core-${COMPILER}-r ${LIBS}

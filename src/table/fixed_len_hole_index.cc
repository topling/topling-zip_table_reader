...
class FixedLenHoleIndexIterTmpl : public FixedLenHoleIndex::Iter {
public:
  using Iter::Iter;
...
#define case_of(SuffixLen) \
  case SuffixLen: return new(mem)FixedLenHoleIndexIterTmpl<SuffixLen>(this)
  case_of( 1);
  case_of( 2);
  case_of( 3);
  case_of( 4);
...
public:
  COIndex* Build(const std::string& keyFilePath,
    ...
    auto raw = malloc(sizeof(FixedLenHoleIndex) + full_cplen);
    ...
    if (tzopt.fixedLenIndexCacheLeafSize >= 64) {
      ROCKS_LOG_DEBUG(iopt->info_log,
        "FixedLenHoleIndex build_cache Start: num_keys = %zd, fixlen = %zd, pref_len = %zd, cplen = %zd",
      ...
      ROCKS_LOG_INFO(iopt->info_log,
        "FixedLenHoleIndex build_cache Finish: num_keys = %zd, fixlen = %zd, pref_len = %zd, cplen = %zd, time = %.3f sec",
  ...
  COIndexUP LoadMemory(fstring mem) const override {
    ...
    auto raw = malloc(sizeof(FixedLenHoleIndex) + pref_len);
    auto fix = new(raw)FixedLenHoleIndex();
    fix->m_header = header;
    fix->m_num_keys = num;
